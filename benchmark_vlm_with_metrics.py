#!/usr/bin/env python3
# -*- coding: utf-8 -*-

"""
Vision-language-model benchmark for mathematical step grading.

The model sees ONLY the image and grading instructions.
Ground-truth JSON step counts/texts/labels are never used to control inference.

Examples:
    python benchmark_vlm.py --all
    python benchmark_vlm.py --type fraction
    python benchmark_vlm.py --file steps/fraction.json
"""

from __future__ import annotations

import argparse
import base64
import json
import logging
import mimetypes
import os
import re
from collections import Counter, defaultdict
from pathlib import Path
from typing import Any

LOGGER = logging.getLogger("benchmark")

ERROR_TYPE_NAMES = {
    "wrong_value": "Carelessly wrote down the wrong value.",
    "wrong_operator": "Wrong operation symbol",
    "wrong_order": "Incorrect parentheses or order of operations",
    "calculation_error": "Calculation error",
    "propagated": "Pre-order error propagation",
}
VALID_ERRORS = set(ERROR_TYPE_NAMES.values())
IMAGE_EXTENSIONS = {".png", ".jpg", ".jpeg", ".webp", ".bmp", ".tif", ".tiff"}

CORRECT_CLASS = "__CORRECT__"
MISSING_CLASS = "__MISSING__"
INVALID_CLASS = "__INVALID__"

PROMPT = """You are grading a mathematical step-by-step solution shown in an image.

First identify the original problem and all visible solution steps by yourself.
Then grade every solution step you can identify, from top to bottom.

For each incorrect step, use exactly one of these error labels:
- "Carelessly wrote down the wrong value."
- "Wrong operation symbol"
- "Incorrect parentheses or order of operations"
- "Calculation error"
- "Pre-order error propagation"

Definitions:
- "Carelessly wrote down the wrong value.": a value, number, or symbol was
  copied, transcribed, substituted, or written incorrectly.
- "Wrong operation symbol": an operation sign/operator was changed or written
  incorrectly.
- "Incorrect parentheses or order of operations": parentheses, precedence, or
  operation order is incorrect.
- "Calculation error": the intended operation is appropriate, but the
  arithmetic result is wrong.
- "Pre-order error propagation": an earlier displayed step already introduced
  an error, and the current transformation is locally valid but continues that
  previous error.

Rules:
- The original problem statement is NOT a solution step.
- Determine the number of solution steps only from the image.
- Do not assume a fixed number of steps.
- Do not omit a step merely because it looks incorrect.
- If a current step introduces a new independent error, use the concrete error
  type instead of "Pre-order error propagation".
- A correct step must have "error_type": null.
- Preserve each recognized step's mathematical text in "step_text".
- Return steps in the same top-to-bottom order as the image.
- Return JSON only, without Markdown fences or explanations.

Required JSON structure:
{
  "steps": [
    {
      "step_text": "the mathematical expression shown in this step",
      "is_correct": true,
      "error_type": null
    }
  ]
}
"""


class ModelRunner:
    """Local Qwen3-VL or OpenAI-compatible vision model."""

    def __init__(self, args: argparse.Namespace) -> None:
        self.backend = args.backend
        self.model_name = args.model
        self.max_new_tokens = args.max_new_tokens

        if self.backend == "qwen-local":
            self._init_qwen(args)
        else:
            self._init_api(args)

    def _init_qwen(self, args: argparse.Namespace) -> None:
        import torch
        from transformers import AutoProcessor, AutoModelForMultimodalLM

        self.torch = torch
        self.processor = AutoProcessor.from_pretrained(self.model_name)

        kwargs: dict[str, Any] = {
            "dtype": "auto",
            "device_map": "auto",
        }
        if args.attn_implementation:
            kwargs["attn_implementation"] = args.attn_implementation

        self.model = AutoModelForMultimodalLM.from_pretrained(
            self.model_name,
            **kwargs,
        )
        self.model.eval()

    def _init_api(self, args: argparse.Namespace) -> None:
        from openai import OpenAI

        api_key = args.api_key or os.getenv("OPENAI_API_KEY")
        base_url = args.api_base or os.getenv("OPENAI_BASE_URL")

        if not api_key:
            raise ValueError(
                "OpenAI-compatible backend requires --api-key "
                "or OPENAI_API_KEY."
            )

        self.client = OpenAI(api_key=api_key, base_url=base_url or None)

    def generate(self, image_path: Path) -> str:
        if self.backend == "qwen-local":
            return self._generate_qwen(image_path)
        return self._generate_api(image_path)

    def _generate_qwen(self, image_path: Path) -> str:
        """Read the local image directly with PIL."""
        from PIL import Image

        with Image.open(image_path) as image_file:
            image = image_file.convert("RGB").copy()

        # Keep the image itself local: create one image placeholder in the
        # chat template, then pass the PIL object directly to the processor.
        messages = [
            {
                "role": "user",
                "content": [
                    {"type": "image"},
                    {"type": "text", "text": PROMPT},
                ],
            }
        ]

        text = self.processor.apply_chat_template(
            messages,
            tokenize=False,
            add_generation_prompt=True,
        )

        inputs = self.processor(
            text=[text],
            images=[image],
            padding=True,
            return_tensors="pt",
        )
        inputs.pop("token_type_ids", None)
        inputs = inputs.to(self.model.device)

        with self.torch.inference_mode():
            output_ids = self.model.generate(
                **inputs,
                max_new_tokens=self.max_new_tokens,
                do_sample=False,
            )

        generated_ids = [
            output[len(input_ids):]
            for input_ids, output in zip(inputs.input_ids, output_ids)
        ]

        return self.processor.batch_decode(
            generated_ids,
            skip_special_tokens=True,
            clean_up_tokenization_spaces=False,
        )[0].strip()

    def _generate_api(self, image_path: Path) -> str:
        """Read a local image and attach it to an API request as Base64."""
        mime_type = mimetypes.guess_type(image_path.name)[0] or "image/png"
        encoded = base64.b64encode(image_path.read_bytes()).decode("ascii")
        data_uri = f"data:{mime_type};base64,{encoded}"

        response = self.client.chat.completions.create(
            model=self.model_name,
            messages=[
                {
                    "role": "user",
                    "content": [
                        {
                            "type": "image_url",
                            "image_url": {"url": data_uri},
                        },
                        {"type": "text", "text": PROMPT},
                    ],
                }
            ],
            temperature=0,
            max_tokens=self.max_new_tokens,
        )

        content = response.choices[0].message.content
        if not content:
            raise ValueError("Model returned an empty response.")
        return content.strip()


def load_label_file(path: Path) -> tuple[Any, list[dict[str, Any]]]:
    """Load label JSON while retaining its top-level structure."""
    with path.open("r", encoding="utf-8") as file:
        raw = json.load(file)

    if isinstance(raw, list):
        records = raw
    elif isinstance(raw, dict) and "question_id" in raw:
        records = [raw]
    elif isinstance(raw, dict):
        records = None
        for key in ("data", "samples", "items", "records"):
            if isinstance(raw.get(key), list):
                records = raw[key]
                break
        if records is None:
            raise ValueError(f"Unsupported JSON structure: {path}")
    else:
        raise ValueError(f"Unsupported JSON structure: {path}")

    if not all(isinstance(item, dict) for item in records):
        raise ValueError(f"Every sample must be a JSON object: {path}")

    return raw, records


def restore_structure(original: Any, predictions: list[dict[str, Any]]) -> Any:
    """Preserve the label file's top-level JSON container."""
    if isinstance(original, list):
        return predictions

    if isinstance(original, dict) and "question_id" in original:
        return predictions[0] if predictions else {}

    result = dict(original)
    for key in ("data", "samples", "items", "records"):
        if isinstance(original.get(key), list):
            result[key] = predictions
            return result

    return predictions


def build_image_index(image_dir: Path) -> dict[str, Path]:
    if not image_dir.is_dir():
        raise FileNotFoundError(f"Image directory not found: {image_dir}")

    index: dict[str, Path] = {}
    for path in image_dir.rglob("*"):
        if path.is_file() and path.suffix.lower() in IMAGE_EXTENSIONS:
            index.setdefault(path.stem, path)

    return index


def find_image(
    sample: dict[str, Any],
    image_dir: Path,
    image_index: dict[str, Path],
) -> Path:
    """Find the local image without inspecting ground-truth steps."""
    for key in ("image", "image_path", "img", "img_path", "filename", "file_name"):
        value = sample.get(key)
        if not value:
            continue

        path = Path(str(value))
        candidate = path if path.is_absolute() else image_dir / path
        if candidate.is_file():
            return candidate.resolve()

    question_id = sample.get("question_id")
    if question_id is None:
        raise FileNotFoundError("Sample has neither an image path nor question_id.")

    image = image_index.get(str(question_id))
    if image is None:
        raise FileNotFoundError(
            f"No image found for question_id={question_id!r} in {image_dir}"
        )

    return image.resolve()


def extract_json_value(text: str) -> Any:
    """Extract the first valid JSON object or array from model output."""
    cleaned = text.strip()

    fence = re.fullmatch(
        r"```(?:json)?\s*(.*?)\s*```",
        cleaned,
        flags=re.IGNORECASE | re.DOTALL,
    )
    if fence:
        cleaned = fence.group(1).strip()

    decoder = json.JSONDecoder()

    for position, char in enumerate(cleaned):
        if char not in "[{":
            continue
        try:
            value, _ = decoder.raw_decode(cleaned[position:])
            return value
        except json.JSONDecodeError:
            continue

    raise ValueError("No valid JSON object or array found in model output.")


def normalize_error(value: Any) -> str | None:
    if value is None:
        return None

    text = str(value).strip()
    if not text or text.lower() in {"null", "none", "correct"}:
        return None

    if text in ERROR_TYPE_NAMES:
        return ERROR_TYPE_NAMES[text]

    for code, label in ERROR_TYPE_NAMES.items():
        if text.lower() in {code.lower(), label.lower()}:
            return label

    return text


def find_step_list(value: Any) -> list[Any]:
    """
    Accept several harmless output wrappers.

    This parsing flexibility is based only on model output, never on GT steps.
    """
    if isinstance(value, list):
        return value

    if not isinstance(value, dict):
        raise ValueError("Model JSON must be an object or array.")

    if isinstance(value.get("steps"), list):
        return value["steps"]

    for key in ("results", "predictions", "grading", "solution_steps"):
        if isinstance(value.get(key), list):
            return value[key]

    # Some models may return one single step object directly.
    if any(
        key in value
        for key in ("step_text", "text", "expression", "is_correct", "correct")
    ):
        return [value]

    raise ValueError("Model JSON does not contain a recognizable step list.")


def parse_bool(value: Any) -> bool:
    if isinstance(value, bool):
        return value

    if isinstance(value, int) and value in (0, 1):
        return bool(value)

    if isinstance(value, str):
        lowered = value.strip().lower()
        if lowered in {"true", "correct", "yes", "1"}:
            return True
        if lowered in {"false", "incorrect", "wrong", "no", "0"}:
            return False

    raise ValueError(f"Cannot parse correctness value: {value!r}")


def parse_prediction(text: str) -> list[list[Any]]:
    """
    Convert model output directly to benchmark step format:
        [step_text, is_correct, error_type]

    The returned number of steps is exactly what can be parsed from the model
    output. It is NOT compared with the ground-truth JSON step count.
    """
    raw_steps = find_step_list(extract_json_value(text))
    predictions: list[list[Any]] = []

    for position, item in enumerate(raw_steps, start=1):
        if isinstance(item, dict):
            step_text = item.get(
                "step_text",
                item.get(
                    "text",
                    item.get("expression", item.get("step", "")),
                ),
            )
            correct_value = item.get(
                "is_correct",
                item.get("correct"),
            )
            error_value = item.get(
                "error_type",
                item.get("error"),
            )

        elif isinstance(item, (list, tuple)):
            # Also accept benchmark-like arrays:
            # ["1 + 2", true, null]
            if len(item) < 2:
                LOGGER.warning(
                    "Skipping unparseable model step at position %d: %r",
                    position,
                    item,
                )
                continue

            if len(item) >= 3:
                step_text, correct_value, error_value = item[:3]
            else:
                step_text = ""
                correct_value, error_value = item[:2]

        else:
            LOGGER.warning(
                "Skipping unparseable model step at position %d: %r",
                position,
                item,
            )
            continue

        try:
            is_correct = parse_bool(correct_value)
        except ValueError:
            LOGGER.warning(
                "Skipping step %d because correctness is invalid: %r",
                position,
                correct_value,
            )
            continue

        error_type = normalize_error(error_value)

        if is_correct:
            error_type = None
        elif error_type not in VALID_ERRORS:
            # Preserve the model's grading decision rather than crashing.
            # Unknown labels are kept as text so the failure is visible in
            # the result file and can be scored later.
            error_type = str(error_type) if error_type is not None else None

        predictions.append(
            [
                "" if step_text is None else str(step_text),
                is_correct,
                error_type,
            ]
        )

    return predictions


def predict_sample(
    sample: dict[str, Any],
    image_path: Path,
    runner: ModelRunner,
    max_attempts: int,
) -> tuple[dict[str, Any], bool]:
    """Grade one image without reading ground-truth steps."""
    last_output = ""
    last_error: Exception | None = None

    for attempt in range(1, max_attempts + 1):
        try:
            last_output = runner.generate(image_path)
            predicted_steps = parse_prediction(last_output)

            # A valid empty steps list is also a valid model prediction.
            prediction = {
                key: value
                for key, value in sample.items()
                if key != "steps"
            }
            prediction["steps"] = predicted_steps
            return prediction, True

        except Exception as exc:
            last_error = exc
            LOGGER.warning(
                "question_id=%s attempt=%d/%d could not parse grading output: %s",
                sample.get("question_id", "?"),
                attempt,
                max_attempts,
                exc,
            )

    LOGGER.error(
        "question_id=%s: saving empty steps after %d failed attempts. "
        "Last error: %s | Last raw output: %r",
        sample.get("question_id", "?"),
        max_attempts,
        last_error,
        last_output[:500],
    )

    prediction = {
        key: value
        for key, value in sample.items()
        if key != "steps"
    }
    prediction["steps"] = []
    return prediction, False


def _step_class(step: list[Any] | None) -> str:
    """Map a predicted/GT step to one confusion-matrix class."""
    if step is None:
        return MISSING_CLASS

    if len(step) < 3:
        return INVALID_CLASS

    if step[1] is True:
        return CORRECT_CLASS

    if step[1] is False:
        error_type = normalize_error(step[2])
        return error_type if error_type in VALID_ERRORS else INVALID_CLASS

    return INVALID_CLASS


def _first_error_index(steps: list[list[Any]]) -> int | None:
    for index, step in enumerate(steps, start=1):
        if len(step) >= 2 and step[1] is False:
            return index
    return None


class MetricsAccumulator:
    """
    Compute metrics only AFTER model inference.

    Ground-truth step counts are never passed to the model and never used to
    request more/fewer grading steps. Position i in the model output is compared
    with position i in GT only during this offline scoring phase.
    """

    def __init__(self) -> None:
        self.num_samples = 0
        self.num_steps = 0
        self.parse_successes = 0
        self.step_count_matches = 0
        self.correctness_hits = 0
        self.error_type_hits_on_gt_errors = 0
        self.gt_error_steps = 0
        self.joint_hits = 0
        self.exact_sample_hits = 0
        self.first_error_hits = 0
        self.missing_prediction_steps = 0
        self.extra_prediction_steps = 0

        self.gt_class_counts: Counter[str] = Counter()
        self.gt_type_hits: Counter[str] = Counter()
        self.confusion: dict[str, Counter[str]] = defaultdict(Counter)

    @staticmethod
    def _safe_div(numerator: int, denominator: int) -> float:
        return numerator / denominator if denominator else 0.0

    def update(
        self,
        ground_truth: dict[str, Any],
        prediction: dict[str, Any],
        parse_success: bool,
    ) -> None:
        gt_steps = ground_truth.get("steps", [])
        pred_steps = prediction.get("steps", [])

        if not isinstance(gt_steps, list):
            raise ValueError("Ground-truth `steps` must be a list.")
        if not isinstance(pred_steps, list):
            pred_steps = []

        self.num_samples += 1
        self.num_steps += len(gt_steps)
        self.parse_successes += int(parse_success)
        self.step_count_matches += int(len(pred_steps) == len(gt_steps))
        self.missing_prediction_steps += max(0, len(gt_steps) - len(pred_steps))
        self.extra_prediction_steps += max(0, len(pred_steps) - len(gt_steps))

        sample_exact = len(pred_steps) == len(gt_steps)

        for index, gt_step in enumerate(gt_steps):
            if not isinstance(gt_step, (list, tuple)) or len(gt_step) < 3:
                raise ValueError(
                    f"Invalid GT step at position {index + 1}: {gt_step!r}"
                )

            gt_step = list(gt_step)
            pred_step = pred_steps[index] if index < len(pred_steps) else None

            gt_correct = gt_step[1]
            gt_error = normalize_error(gt_step[2])
            pred_correct = pred_step[1] if pred_step is not None and len(pred_step) >= 2 else None
            pred_error = (
                normalize_error(pred_step[2])
                if pred_step is not None and len(pred_step) >= 3
                else None
            )

            if pred_correct is gt_correct:
                self.correctness_hits += 1

            if gt_correct is False:
                self.gt_error_steps += 1
                if pred_correct is False and pred_error == gt_error:
                    self.error_type_hits_on_gt_errors += 1
                    if gt_error in VALID_ERRORS:
                        self.gt_type_hits[str(gt_error)] += 1

            joint_hit = (
                pred_correct is gt_correct
                and pred_error == gt_error
            )
            if joint_hit:
                self.joint_hits += 1
            else:
                sample_exact = False

            gt_class = _step_class(gt_step)
            pred_class = _step_class(pred_step)
            self.gt_class_counts[gt_class] += 1
            self.confusion[gt_class][pred_class] += 1

        if sample_exact:
            self.exact_sample_hits += 1

        gt_first_error = _first_error_index([list(step) for step in gt_steps])

        # Match the earlier benchmark semantics: if the model omitted any GT
        # position, FEIA is counted as incorrect. Extra trailing steps do not
        # by themselves invalidate the first-error position.
        if len(pred_steps) < len(gt_steps):
            pred_first_error: int | None | str = MISSING_CLASS
        else:
            pred_first_error = _first_error_index(pred_steps)

        if pred_first_error == gt_first_error:
            self.first_error_hits += 1

    def merge(self, other: "MetricsAccumulator") -> None:
        for attr in (
            "num_samples",
            "num_steps",
            "parse_successes",
            "step_count_matches",
            "correctness_hits",
            "error_type_hits_on_gt_errors",
            "gt_error_steps",
            "joint_hits",
            "exact_sample_hits",
            "first_error_hits",
            "missing_prediction_steps",
            "extra_prediction_steps",
        ):
            setattr(self, attr, getattr(self, attr) + getattr(other, attr))

        self.gt_class_counts.update(other.gt_class_counts)
        self.gt_type_hits.update(other.gt_type_hits)
        for gt_class, counts in other.confusion.items():
            self.confusion[gt_class].update(counts)

    def to_dict(self) -> dict[str, Any]:
        per_error_type = {}
        for error_name in ERROR_TYPE_NAMES.values():
            count = self.gt_class_counts[error_name]
            hits = self.gt_type_hits[error_name]
            per_error_type[error_name] = {
                "count": count,
                "accuracy": self._safe_div(hits, count),
            }

        confusion_dict = {
            gt_class: dict(pred_counts)
            for gt_class, pred_counts in sorted(self.confusion.items())
        }

        return {
            "num_samples": self.num_samples,
            "num_steps": self.num_steps,
            "parse_success_rate": self._safe_div(
                self.parse_successes,
                self.num_samples,
            ),
            "step_count_match_rate": self._safe_div(
                self.step_count_matches,
                self.num_samples,
            ),
            "step_correctness_accuracy (SCA)": self._safe_div(
                self.correctness_hits,
                self.num_steps,
            ),
            "error_type_accuracy_on_gt_error_steps (ETA)": self._safe_div(
                self.error_type_hits_on_gt_errors,
                self.gt_error_steps,
            ),
            "joint_step_accuracy (JSA)": self._safe_div(
                self.joint_hits,
                self.num_steps,
            ),
            "sample_exact_match (SEM)": self._safe_div(
                self.exact_sample_hits,
                self.num_samples,
            ),
            "first_error_index_accuracy (FEIA)": self._safe_div(
                self.first_error_hits,
                self.num_samples,
            ),
            "ground_truth_error_steps": self.gt_error_steps,
            "missing_prediction_steps": self.missing_prediction_steps,
            "extra_prediction_steps": self.extra_prediction_steps,
            "per_error_type": per_error_type,
            "confusion_matrix": confusion_dict,
        }


def evaluate_file(
    label_file: Path,
    data_root: Path,
    result_root: Path,
    runner: ModelRunner,
    limit: int | None,
    max_attempts: int,
) -> MetricsAccumulator:
    """Evaluate one category and save predictions plus category metrics."""
    category = label_file.stem
    image_dir = data_root / category
    output_file = result_root / f"{category}.json"
    metrics_file = result_root / f"{category}_metrics.json"

    original, samples = load_label_file(label_file)
    if limit is not None:
        samples = samples[:limit]

    image_index = build_image_index(image_dir)
    predictions: list[dict[str, Any]] = []
    metrics = MetricsAccumulator()

    LOGGER.info(
        "Testing %s | samples=%d | images=%s",
        category,
        len(samples),
        image_dir,
    )

    try:
        from tqdm import tqdm
        iterator = tqdm(samples, desc=category, unit="sample")
    except ImportError:
        iterator = samples

    for sample in iterator:
        question_id = sample.get("question_id", "?")
        parse_success = False

        try:
            image_path = find_image(sample, image_dir, image_index)
            prediction, parse_success = predict_sample(
                sample=sample,
                image_path=image_path,
                runner=runner,
                max_attempts=max_attempts,
            )

        except Exception as exc:
            LOGGER.error(
                "%s: question_id=%s failed before/while inference: %s",
                category,
                question_id,
                exc,
            )
            prediction = {
                key: value
                for key, value in sample.items()
                if key != "steps"
            }
            prediction["steps"] = []

        # GT is consulted only here, after prediction already exists.
        metrics.update(sample, prediction, parse_success)
        predictions.append(prediction)

    result_root.mkdir(parents=True, exist_ok=True)

    output = (
        restore_structure(original, predictions)
        if limit is None
        else predictions
    )

    with output_file.open("w", encoding="utf-8") as file:
        json.dump(output, file, ensure_ascii=False, indent=2)

    with metrics_file.open("w", encoding="utf-8") as file:
        json.dump(metrics.to_dict(), file, ensure_ascii=False, indent=2)

    LOGGER.info("Saved predictions: %s", output_file)
    LOGGER.info("Saved metrics: %s", metrics_file)
    return metrics


def select_label_files(args: argparse.Namespace) -> list[Path]:
    label_root = Path(args.label_root)

    if args.all:
        files = sorted(label_root.glob("*.json"))
        if not files:
            raise FileNotFoundError(f"No JSON files found in {label_root}")
        return files

    path = (
        label_root / f"{args.type}.json"
        if args.type
        else Path(args.file)
    )

    if not path.is_file():
        raise FileNotFoundError(f"Label file not found: {path}")

    return [path]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="VLM benchmark for mathematical step-by-step grading."
    )

    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument("--all", action="store_true")
    mode.add_argument("--type", help="Example: --type fraction")
    mode.add_argument("--file", help="Example: --file steps/fraction.json")

    parser.add_argument("--data-root", default="printvision")
    parser.add_argument("--label-root", default="steps")
    parser.add_argument("--result-root", default="results")

    parser.add_argument(
        "--backend",
        choices=("qwen-local", "openai-compatible"),
        default="qwen-local",
    )
    parser.add_argument(
        "--model",
        default="/home/shared/winston/model/Qwen/Qwen2___5-VL-7B-Instruct",
    )
    parser.add_argument("--max-new-tokens", type=int, default=1024)
    parser.add_argument(
        "--attn-implementation",
        choices=("sdpa", "flash_attention_2", "eager"),
        default=None,
    )

    parser.add_argument("--api-base", default=None)
    parser.add_argument("--api-key", default=None)

    parser.add_argument(
        "--max-attempts",
        type=int,
        default=3,
        help="Retry only when the model output cannot be parsed; GT is not used.",
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=None,
        help="Evaluate only the first N samples for debugging.",
    )
    parser.add_argument("--verbose", action="store_true")

    return parser.parse_args()


def main() -> None:
    args = parse_args()

    logging.basicConfig(
        level=logging.DEBUG if args.verbose else logging.INFO,
        format="%(asctime)s | %(levelname)s | %(message)s",
    )

    data_root = Path(args.data_root).resolve()
    result_root = Path(args.result_root).resolve()
    result_root.mkdir(parents=True, exist_ok=True)
    label_files = select_label_files(args)

    LOGGER.info("Loading model once: %s", args.model)
    runner = ModelRunner(args)

    overall = MetricsAccumulator()
    by_category: dict[str, dict[str, Any]] = {}

    for label_file in label_files:
        category_metrics = evaluate_file(
            label_file=label_file.resolve(),
            data_root=data_root,
            result_root=result_root,
            runner=runner,
            limit=args.limit,
            max_attempts=args.max_attempts,
        )
        overall.merge(category_metrics)
        by_category[label_file.stem] = category_metrics.to_dict()

    summary = {
        "overall": overall.to_dict(),
        "by_category": by_category,
    }

    summary_file = result_root / "metrics.json"
    with summary_file.open("w", encoding="utf-8") as file:
        json.dump(summary, file, ensure_ascii=False, indent=2)

    LOGGER.info("Saved summary metrics: %s", summary_file)
    print(json.dumps(summary, ensure_ascii=False, indent=2))


if __name__ == "__main__":
    main()
