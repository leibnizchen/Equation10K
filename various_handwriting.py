...
if __name__ == '__main__':
    ...
    # QUESTION_TYPES =["random_combination"]
    for q_t in QUESTION_TYPES:
        folder = f"{"datavision"}/{q_t}"
        if not os.path.exists(folder):
            os.makedirs(folder)
        with open(f"{"steps"}/{q_t}.json", "r", encoding="utf-8") as f:
            dataset = json.load(f)

        for data in dataset:
            generator = handwrite_generator(hand=random.randint(0,11))
            ...
            text =   "\n".join( f"={str(step[0]).replace("**","^").replace("*","X").replace("/","/").replace("×","X").replace("÷","/").strip('=')}" for step in data["steps"])
            text= text.replace(" ","")

            generator.generate_image(data["question"],text,f"{"datavision"}/{q_t}/{data["question_id"]}.png")
